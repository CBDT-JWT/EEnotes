...
**变长顺序存储**：根据实际需要分配存储区域的规模，使用终结符（如`\0`），避免截断并提高空间利用率。
...
**蛮力法**：T的每个位置都作为可能的起始位置，与模式串P逐次进行比较把目标串，如果相等，则匹配成功，过程终止；如果不成功，就一直比较下去，知道遍历目标串的所有位置，匹配失败，过程终止。
...
**KMP算法**：认为在匹配过程中，当模式𝑃不匹配时，应尽量向右移动最大距离，以避免重复比较。

![alt text](assets/dsa_image-15.png)

- 步骤1: 模式串P预处理
- 步骤2: 每次发生失配时，根据最后一个匹配位置的部分匹配串长度向右移动模式串，使具有部分匹配长度的前缀对准相应的后缀所处的位置

![alt text](assets/dsa_image-16.png)

`Next`函数：计算模式串P每个位置部分匹配串的长度
...
![alt text](assets/dsa_image-17.png)
...
![alt text](assets/dsa_image-18.png)

- 根据模式串 $P$ 预先计算字母表的 Last-Occurrence 函数 $L:\Sigma\rightarrow\mathbb{Z}$

- 对于每个 $\xi\in\Sigma$，$L$ 按以下规则计算：
  - 如果 $\xi$ 在 $P$ 中出现，那么 $L$ 把 $\xi$ 映射到它在 $P$ 中出现的最后位置；
  - 否则，即 $\xi$ 不在 $P$ 中，那么 $L$ 把 $\xi$ 映射到 $-1$。
